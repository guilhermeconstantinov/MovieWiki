<template>
    <div>
        ...
        <Content type="movie"  @search="search" :items="movies" label="List Movies"/>
...
export default {
    ...
    methods:{
        ...
        getApi(range){
            return axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${this.apiKey}&language=pt-BR&sort_by=release_date.desc&include_adult=false&include_video=false&page=${range}&release_date.lte=2020-12-30`)
                .then(res => res.data)
        }
    } 
}
